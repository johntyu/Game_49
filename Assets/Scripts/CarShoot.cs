	...
	void Update () {
		...
		if(targetEnemy != null){
			if (fireTime > fireDelay && !isLockdown) {
				...
				if(gameObject.layer == 9) {
					instance = Instantiate(bullet, transform.position, bullet.transform.rotation) as GameObject;
				}
				else {
					instance = Instantiate(bullet2, transform.position, bullet2.transform.rotation) as GameObject;
				}
